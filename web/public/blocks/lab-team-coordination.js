// Lab Link V3 - Team Coordination Block

import LabBlock from '../components/lab-block.js';
import { TEAM_COORDINATION_MANIFEST } from '../registry/block-manifests.js';

class LabTeamCoordination extends LabBlock {
  constructor() {
    super();
    this.manifest = TEAM_COORDINATION_MANIFEST;
    this.state = { activeSubtab: 'members', members: [], roles: [], tasks: [] };
  }

  init() {
    this.subscribe('data:updated', (data) => {
      if (data.type === 'team') this.loadTeam();
    });
    this.loadTeam();
  }
...
customElements.define('lab-team-coordination', LabTeamCoordination);
export default LabTeamCoordination;

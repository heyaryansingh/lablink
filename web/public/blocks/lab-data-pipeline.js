// Lab Link V3 - Data Pipeline Block

import LabBlock from '../components/lab-block.js';
import { DATA_PIPELINE_MANIFEST } from '../registry/block-manifests.js';

class LabDataPipeline extends LabBlock {
  constructor() {
    super();
    this.manifest = DATA_PIPELINE_MANIFEST;
    this.state = { activeSubtab: 'active', activePipelines: [], queued: [], completed: [] };
  }

  init() {
    this.subscribe('data:updated', (data) => {
      if (data.type === 'pipelines') this.loadPipelines();
    });
    this.loadPipelines();
  }
  ...
  async loadPipelines() {
    try {
      ...
      this.setState({
        ...
        queued: data.queuedPipelines || [],
        completed: data.completedPipelines || [],
      });
    } catch (error) {
      this.setError(error);
    }
...
customElements.define('lab-data-pipeline', LabDataPipeline);
export default LabDataPipeline;

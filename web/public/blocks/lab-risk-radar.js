// Lab Link V3 - Risk Radar Block

import LabBlock from '../components/lab-block.js';
import { RISK_RADAR_MANIFEST } from '../registry/block-manifests.js';

class LabRiskRadar extends LabBlock {
  constructor() {
    super();
    this.manifest = RISK_RADAR_MANIFEST;
    this.state = { activeSubtab: 'critical', critical: [], medium: [], mitigated: [] };
  }

  init() {
    this.subscribe('data:updated', (data) => {
      if (data.type === 'risks') this.loadRisks();
    });
    this.loadRisks();
  }
  ...
  async loadRisks() {
    try {
      ...
      this.setState({
        ...
        medium: data.mediumRisks || [],
        mitigated: data.mitigatedRisks || [],
      });
    } catch (error) {
      this.setError(error);
    }
...
customElements.define('lab-risk-radar', LabRiskRadar);
export default LabRiskRadar;

// Lab Link V3 - Inbox Signals Block

import LabBlock from '../components/lab-block.js';
import { INBOX_SIGNALS_MANIFEST } from '../registry/block-manifests.js';

class LabInboxSignals extends LabBlock {
  constructor() {
    super();
    this.manifest = INBOX_SIGNALS_MANIFEST;
    this.state = { activeSubtab: 'unread', unread: [], important: [], archived: [] };
  }

  init() {
    this.subscribe('data:updated', (data) => {
      if (data.type === 'inbox') this.loadInbox();
    });
    this.loadInbox();
  }
  ...
  async loadInbox() {
    try {
      ...
      this.setState({
        ...
        important: data.importantMessages || [],
        archived: data.archivedMessages || [],
      });
    } catch (error) {
      this.setError(error);
    }
...
customElements.define('lab-inbox-signals', LabInboxSignals);
export default LabInboxSignals;

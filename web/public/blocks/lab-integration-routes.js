// Lab Link V3 - Integration Routes Block

import LabBlock from '../components/lab-block.js';
import { INTEGRATION_ROUTES_MANIFEST } from '../registry/block-manifests.js';

class LabIntegrationRoutes extends LabBlock {
  constructor() {
    super();
    this.manifest = INTEGRATION_ROUTES_MANIFEST;
    this.state = { activeSubtab: 'active', active: [], pending: [], available: [] };
  }

  init() {
    this.subscribe('data:updated', (data) => {
      if (data.type === 'integrations') this.loadIntegrations();
    });
  ...
  async loadIntegrations() {
    try {
      ...
      this.setState({
        active: data.activeIntegrations || [],
        pending: data.pendingIntegrations || [],
        available: data.availableIntegrations || [],
      });
    } catch (error) {
      this.setError(error);
    }
...
customElements.define('lab-integration-routes', LabIntegrationRoutes);
export default LabIntegrationRoutes;

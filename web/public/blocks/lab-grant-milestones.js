// Lab Link V3 - Grant Milestones Block

import LabBlock from '../components/lab-block.js';
import { GRANT_MILESTONES_MANIFEST } from '../registry/block-manifests.js';

class LabGrantMilestones extends LabBlock {
  constructor() {
    super();
    this.manifest = GRANT_MILESTONES_MANIFEST;
    this.state = { activeSubtab: 'active', activeGrants: [], upcoming: [], completed: [] };
  }

  init() {
    this.subscribe('data:updated', (data) => {
      if (data.type === 'grants') this.loadGrants();
    });
    this.loadGrants();
  }
  ...
  async loadGrants() {
    try {
      ...
      this.setState({
        ...
        upcoming: data.upcomingMilestones || [],
...
customElements.define('lab-grant-milestones', LabGrantMilestones);
export default LabGrantMilestones;

// Lab Link V3 - Custom Sections Block

import LabBlock from '../components/lab-block.js';
import { CUSTOM_SECTIONS_MANIFEST } from '../registry/block-manifests.js';

class LabCustomSections extends LabBlock {
  constructor() {
    super();
    this.manifest = CUSTOM_SECTIONS_MANIFEST;
    this.state = { activeSubtab: 'my-sections', mySections: [], templates: [], shared: [] };
  }

  init() {
    this.subscribe('data:updated', (data) => {
      if (data.type === 'custom-sections') this.loadSections();
    });
    this.loadSections();
  }
  ...
  async loadSections() {
    try {
      ...
      this.setState({
        mySections: data.customSections || [],
        templates: data.sectionTemplates || [],
        shared: data.sharedSections || [],
      });
    } catch (error) {
      this.setError(error);
    }
...
customElements.define('lab-custom-sections', LabCustomSections);
export default LabCustomSections;
